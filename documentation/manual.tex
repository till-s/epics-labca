...
\begin{document}
\maketitle
{\hspace*{\fill}{\small\verb$Id: manual.tex,v 1.22 2004/06/20 04:37:11 till Exp $}\hspace*{\fill}}
...
\scilab{}/\matlab{} are processing. You only need to poll the library
if the data are ready (using {\tt lcaNewMonitorValue()}).

Under 3.13, however, the user must periodically call {\tt lcaDelay()} or
an other  \sca{} routine in order to yield the CPU to the channel access
library.
\subsubsection{Parameters}
\begin{description}
\item[timeout]
